...
# --- 4. CONTROLLO SE LA TAPPA È IN CORSO (LOGICA TABELLA ORIZZONTALE) ---
if sel_tappa['id_stage'] in current_ids:
    st.info(f"🚀 Formazioni schierate per: **{sel_tappa['race_name']}**")
    
    # Query alla vista che contiene display_name e rider_name_short
    res_global = supabase.table("view_user_pick")\
        .select("display_name, rider_name_short, id_slot")\
        .eq("id_stage", sel_tappa['id_stage']).execute()
    ...
    if res_global.data:
        ...
        # Trasformiamo la tabella da verticale a orizzontale (Pivot)
        # Indice: Nome Utente | Colonne: Slot 1, 2, 3... | Valori: Nome Corridore
        df_pivot = df_raw.pivot(index='display_name', columns='id_slot', values='rider_name_short')
        
        # Rinominiamo le colonne per chiarezza (es. "Slot 1", "Slot 2", ecc.)
        df_pivot.columns = [f"Rider {int(col)}" for col in df_pivot.columns]
        
        # Pulizia: riempiamo eventuali slot vuoti con "-" e resettiamo l'indice per mostrare il nome utente come colonna
        df_final = df_pivot.fillna("-").reset_index()
        df_final.rename(columns={'display_name': 'Utente'}, inplace=True)
...
# --- 5. LOGICA INSERIMENTO (RESTO DEL CODICE INVARIATO) ---
res_existing = supabase.table("fact_user_pick")\
    .select("id_slot, id_rider")\
    .eq("id_user", user_id)\
    .eq("id_stage", sel_tappa['id_stage']).execute()
