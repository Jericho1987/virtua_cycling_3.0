...
def render_sidebar():
    """Disegna la sidebar. Usa una chiave univoca per il CookieManager."""
    # Chiave differente da Home.py per evitare DuplicateElementKey
    cm_sidebar = stx.CookieManager(key="sidebar_manager_unique")
    ...
    with st.sidebar:
        ...
        with col1:
            if st.button("Profilo ⚙️", key="btn_side_prof", use_container_width=True):
                st.switch_page("pages/07_modifica_profilo.py")
        with col2:
            if st.button("Esci 🚪", key="btn_side_logout", use_container_width=True):
                cm_sidebar.delete("supabase_refresh_token")
